...
def ask_for_player_move(display_mode):
    """Let the player select a color to pain the upper left tile."""
    while True:
        bext.fg("white")
...
def change_tile(tile_type, board, x, y, char_to_change=None):
    """Change the color/shape of a tile using the recursive flood fill algorith"""

    if x == 0 and y == 0:
        char_to_change = board[(x, y)]
        if tile_type == char_to_change:
            return  # Base Case: Already in the same tile.
